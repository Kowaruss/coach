class BetCalculator {
    ...
    // Установка фильтра рулетки
    setRouletteFilter(filter) {
        this.rouletteFilter = filter;
    }
    
    // Установка фильтра соседей
    setNeighborsFilter(filter) {
        this.neighborsFilter = filter;
    }
    
    // Выбор случайной рулетки с учетом фильтра
    getRandomRoulette() {
        let availableRoulettes = this.rouletteTypes;
        
        if (this.rouletteFilter !== 'all') {
            availableRoulettes = this.rouletteTypes.filter(roulette => 
                roulette.name === this.rouletteFilter
            );
        }
        
        return availableRoulettes[Math.floor(Math.random() * availableRoulettes.length)];
    }
    
    // Выбор случайного количества соседей с учетом фильтра
    getRandomNeighborCount() {
        let availableNeighbors = this.neighborCounts;
        
        if (this.neighborsFilter !== 'all') {
            availableNeighbors = [parseInt(this.neighborsFilter)];
        }
        
        return availableNeighbors[Math.floor(Math.random() * availableNeighbors.length)];
    }
